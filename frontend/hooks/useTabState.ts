...
// Tab 1: Notary State
// =====================================================================
export function useNotaryTab(opts: { onLibraryRefresh?: () => void; token?: string | null } = {}) {
  ...
  const [astVerification, setAstVerification] = useState<AstVerificationResult | null>(null);
  ...
  const [notarySuccess, setNotarySuccess] = useState<NotarySuccessResult | null>(null);
...
export function useInspectorTab(opts: { walletAddress?: string | null; onLicense?: (c: string | null) => void } = {}) {
  ...
  const [inspectedDoc, setInspectedDoc] = useState<InspectedDoc | null>(null);
  const [ipNftMinting, setIpNftMinting] = useState<boolean>(false);
  const [ipNftResult, setIpNftResult] = useState<IpNftResult | null>(null);

  const handleInspect = async (code: string) => {
    try {
      const res = await fetch(`${getApiBase()}/notary/certificate/${code}`);
      const data = (await res.json()) as NotaryCertificateLayer;
...
export function useZkDiscoveryTab(opts: { orcid?: string; authorName?: string } = {}) {
  ...
  const [zkRevealResult, setZkRevealResult] = useState<ZkRevealResult | null>(null);
  ...
  const handleZkCommit = async () => {
    try {
      ...
      const data = (await res.json()) as ZkCommitResult;
  ...
  const handleZkReveal = async () => {
    try {
      ...
      const data = (await res.json()) as ZkRevealResult;
...
export function usePassportTab() {
  ...
  const [passportData, setPassportData] = useState<ScholarProfile | null>(null);
...
export function useReviewTab(opts: { token?: string | null } = {}) {
  ...
  const [reviewerReputation, setReviewerReputation] = useState<Record<string, unknown> | null>(null);
  const [claimResult, setClaimResult] = useState<Record<string, unknown> | null>(null);

  const fetchReputation = useCallback(async (orcid: string) => {
    try {
      const res = await fetch(`${getApiBase()}/api/v1/review/reputation/${encodeURIComponent(orcid)}`);
      if (res.status === 404) {
        setReviewerReputation(null);
        return;
      }
      const data = (await res.json()) as Record<string, unknown>;
      setReviewerReputation(data);
    } catch {
      setReviewerReputation(null);
    }
  }, []);
  ...
  const handleSubmitReview = async () => {
    try {
      ...
      setClaimResult(null);
      if (data.review_id) {
        // Агрегаты обновляются сразу после отправки (приватность слепых рецензий сохраняется).
        void fetchReputation(revOrcid);
      }
    } catch {}
  };

  const handleClaimAttestation = async () => {
  ...
  return {
    ...
    reviewerReputation, fetchReputation, claimResult, handleClaimAttestation,
  };
}
...
export function useMaasTab() {
  ...
  const [maasResult, setMaasResult] = useState<unknown>(null);
  const [fhirLoading, setFhirLoading] = useState<boolean>(false);
  const [fhirResult, setFhirResult] = useState<unknown>(null);
...
export function useAmanatTab() {
  ...
  const [fiatInvoiceResult, setFiatInvoiceResult] = useState<FiatInvoiceResult | null>(null);
  ...
  const handleGenerateFiatInvoice = async () => {
    ...
    try {
      ...
      const data = (await res.json()) as FiatInvoiceResult;
...
export function useCourtTab(opts: { t?: TranslationDict; scholarToken?: string | null } = {}) {
  ...
  const [courtDisputeResult, setCourtDisputeResult] = useState<CourtCase | null>(null);
...
// =====================================================================
export function useVampireTab(opts: { onLibraryRefresh?: () => void; t?: TranslationDict; token?: string | null } = {}) {
  const t = opts.t;
  const [vampireQuery, setVampireQuery] = useState<string>("oncology neuro-immune axis");
  const [vampireSource, setVampireSource] = useState<VampireSource>("all");
  const [vampireSearching, setVampireSearching] = useState<boolean>(false);
  const [vampireResults, setVampireResults] = useState<VampireWork[]>([]);
